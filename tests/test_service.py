...
def mock_extractor():
    ...
    ext.parse_video_id = MagicMock(return_value="test123")
    return ext
...
@pytest.fixture
def service(repo, mock_extractor, wiki_engine, mock_llm, mock_scene_extractor, mock_vision_describer):
...
class TestAddVideo:
    def test_add_video_success(self, service):
        video = service.add_video("https://youtube.com/watch?v=test123")
        assert video.video_id == "test123"
        assert video.title == "Test Video"

    def test_add_video_saves_to_repo(self, service, repo):
        service.add_video("https://youtube.com/watch?v=test123")
    ...
    def test_add_video_text_only(self, service, mock_scene_extractor):
        service.add_video("https://youtube.com/watch?v=test123", text_only=True)
    ...
    def test_add_video_full_analysis(self, service, mock_scene_extractor, mock_vision_describer):
        service.add_video("https://youtube.com/watch?v=test123", text_only=False)
...
class TestListAndInfo:
    ...
    def test_get_info(self, service):
        service.add_video("https://youtube.com/watch?v=test123")
        video = service.get_info("test123")
        assert video.title == "Test Video"
...
class TestWikiShow:
    def test_wiki_show(self, service):
        service.add_video("https://youtube.com/watch?v=test123")
        page = service.wiki_show("video-test123")
...
class TestWikiHistory:
    def test_wiki_history_empty(self, service):
        service.add_video("https://youtube.com/watch?v=test123")
        history = service.wiki_history("video-test123")
...
class TestAskVideo:
    def test_ask_single_video(self, service, mock_llm):
        service.add_video("https://youtube.com/watch?v=test123")
        answer = service.ask_video("test123", "What is this about?")
    ...
    def test_ask_videos_multiple(self, service, mock_llm, mock_extractor):
        ...
        mock_extractor.extract.return_value = Video(
            video_id="test456", title="V2", channel="C", duration=60.0,
            transcript=[TranscriptSegment(start=0.0, duration=5.0, text="Second video.")],
        )
        mock_extractor.parse_video_id = MagicMock(return_value="test456")
        service.add_video("https://youtube.com/watch?v=test456")
        answer = service.ask_videos(["test123", "test456"], "Compare them")
        assert len(answer) > 0
...
class TestClassify:
    def test_classify_video(self, service, mock_llm):
        ...
        tags = service.classify_video("test123")
        assert tags == ["new-tag"]
...
class TestResolveVideo:
    def test_resolve_by_id(self, service):
        service.add_video("https://youtube.com/watch?v=test123")
        video = service.resolve_video("test123")
        assert video.video_id == "test123"
...
class TestFrames:
    ...
    def test_get_frame_by_query_matches(self, service):
        ...
        with patch.object(service._frame_extractor, "extract_frame", return_value="/tmp/frame.jpg"):
            result = service.get_frame_by_query("test123", "hello")
